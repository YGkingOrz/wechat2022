...
import sys
sys.path.append("..")
from data_qq.masklm import MaskLM, MaskVideo, ShuffleVideo
...
#         print('---------------cfg[HIDDEN_SIZE]',cfg['HIDDEN_SIZE'])
...
# #             print('vm_output',vm_output.shape)
...
#         print('倒数了',hidden_states.shape)
        hidden_states = self.dense(hidden_states)
#         print('倒数了1',hidden_states.shape)
...
class VisualLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self.decoder = nn.Linear(config.hidden_size, 768, bias=False)#最终纬度1536
        self.bias = nn.Parameter(torch.zeros(768))#1536
...
#         print('倒数hidden_states',hidden_states.shape)
        return hidden_states
...
class UniBert(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.video_fc = torch.nn.Linear(768, 768)#1536,768,要把图像转换到能过embedding的地步
...
#         print(video_feature.shape)
        ...
        encoder_outputs = self.encoder(embedding_output, attention_mask=mask, output_hidden_states = True)['last_hidden_state']#x
